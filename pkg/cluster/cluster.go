...
import (
	...
	api "etcd-operator/pkg/apis/samplecrd/v1"
	"etcd-operator/pkg/generated/clientset/versioned"
	"etcd-operator/pkg/util/etcdutil"
	"etcd-operator/pkg/util/k8sutil"
	"etcd-operator/pkg/util/retryutil"
...
func New(config Config, cl *api.EtcdCluster) *Cluster {
	lg := logrus.WithFields(logrus.Fields{"pkg": "cluster", "cluster-name": cl.Name, "cluster-namespace": cl.Namespace})
...
func (c *Cluster) updateCRStatus() error {
	...
	newCluster, err := c.config.EtcdCRCli.SamplecrdV1().EtcdClusters(c.cluster.Namespace).Update(c.cluster)
	if err != nil {
